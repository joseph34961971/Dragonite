...
def unsupervised_loco_edit(
    editor,
    zt: torch.Tensor,
    t: torch.Tensor,
    t_idx: int,
    for_prompt_emb: torch.Tensor,
    edit_prompt_emb: torch.Tensor,
    null_prompt_emb: torch.Tensor,
    mask: torch.Tensor,
    semantic_k: int = 0,
    nullspace_rank: int = 5,
    edit_strength: float = 1.0,
    mode: str = "null+(for-null)",
    # parameters for the power-method SVD in local_encoder_decoder_pullback_zt:
    pca_rank: int = 50,
    chunk_size: int = 25,
    min_iter: int = 10,
    max_iter: int = 100,
    ...
    """
    Perform one-step Unsupervised LOCO Edit and return edited x0.
    
    Args:
       editor:          an instance of EditStableDiffusion
       zt:              latent tensor at edit timestep (shape [B, C, H, W])
       t:               corresponding scalar timestep value
       t_idx:           index of t in editor.scheduler.timesteps
       for_prompt_emb:  embedding of the original prompt
       edit_prompt_emb: embedding of the edit prompt
       null_prompt_emb: embedding of the null prompt
       mask:            Boolean mask Ω (shape [H, W] or [1, H, W])
       semantic_k:      which semantic singular vector to pick (0-based)
       nullspace_rank:  how many null-space directions to compute
       edit_strength:   λ, amount to move along the semantic vector
       mode:            guidance mode passed to the calls
       pca_rank, … :    parameters for the local SVD power method

    Returns:
       x0_edited:    torch.Tensor of shape [B, H, W, 3], your edited image(s)
    """
    # 1) text-supervised semantic direction on Ω
    v_modify = editor.get_delta_zt_via_grad(
        zt, t, t_idx,
        for_prompt_emb, edit_prompt_emb, null_prompt_emb,
        mask=mask, mode=mode
    )      # shape [r, C, H, W], where r ≥ semantic_k+1

    # 2) null-space basis on complement Ω̄
    u_null, s_null, vT_null = editor.local_encoder_decoder_pullback_zt(
        zt, t, t_idx,
        for_prompt_emb, edit_prompt_emb, null_prompt_emb,
        op=None, block_idx=None,
        pca_rank=nullspace_rank, chunk_size=chunk_size,
        min_iter=min_iter, max_iter=max_iter,
        convergence_threshold=convergence_threshold,
        mask=~mask,     # complement of Ω
        mode="null+(for-null)"
    )
    # vT_null: shape [nullspace_rank, C*H*W]

    # 3) pick the k-th semantic vector and project out Ω̄–components
    v = v_modify[semantic_k].view(-1)      # flatten to (C*H*W,)
    # projection onto span(vT_null):
    proj = vT_null.T @ (vT_null @ v)       # (C*H*W,)
    v_p = v - proj
    v_p = v_p / v_p.norm()

    # # 4) apply one DDIM step of size edit_strength along v_p
    # vk = v_p.view_as(zt)                   # reshape back to (C,H,W)
    # zt_edit = zt + edit_strength * vk.unsqueeze(0)

    # # 5) run forward DDIM from t_idx → t=0
    # _, x0_edited = editor.DDIMforwardsteps(
    #     zt_edit,
    #     t_start_idx=t_idx,
    #     t_end_idx=-1,
